...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <div class="my-auto">
            <h5 class="page-title fs-21 mb-1">{{ __('Purchase Details') }}</h5>
        </div>
                ...
                <div class="card card-invoice">
                    <div class="card-body">
                        <div class="invoice-header">
                            ...
                            {!! generateQRCode(route('purchase.show', $purchase->id)) !!}
                        ...
                        <div class="row mt-4">
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>{{ __('Purchase Date') }}:</span> <span>{{ $purchase->date ?? 'N/A' }}</span></p>
                                <p class="invoice-info-row"><span>{{ __('Paying Date') }}:</span> <span>{{ $purchase->paying_date ?? 'N/A' }}</span></p>
                            </div>
                        </div>
                                ...
                                <tr>
                                    <td class="align-top" colspan="2" rowspan="4">
                                        <div class="invoice-notes">
                                            ...
                                            <p class="fw-normal fs-13">{{ $purchase->purchase_note }}</p>
                                        </div><!-- invoice-notes -->
                                    </td>
                                    <td>{{ __('Sub-Total') }}</td>
                                    <td colspan="2">{{ showAmount($purchase->payable_amount) }}</td>
                                </tr>
                                <tr>
                                    <td>{{ __('Tax') }} ({{ $purchase->tax_amount }}%)</td>
                                    <td colspan="2">{{ showAmount(taxAmount($purchase->total_price, $purchase->tax_amount)) }}</td>
                                </tr>
                                <tr>
                                    <td>{{ __('Discount') }}</td>
                                    <td colspan="2">-{{ showAmount($purchase->discount) }}</td>
                                </tr>
                                <tr>
                                    <td class=" text-uppercase tx-inverse">{{ __('Total Due') }}</td>
                                    <td colspan="2">
                                        <h4 class="text-danger">{{ showAmount($purchase->dueAmount()) }}</h4>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
